...
| III-A Eq.(15–17)   | 内積から距離への変換                      | ✅ 実装  | 関数 ⚠️ d(x, y) = √(2 − 2⟨x｜y⟩) の式は明示的には登場しないが、正規化ベクトルに基づくこの恒等式を前提としてアルゴリズムが設計されている。ユークリッド距離の2乗は振幅（例：sin²θ）として符号化されており、量子内積に基づく距離構造を暗黙的に利用している。   |
